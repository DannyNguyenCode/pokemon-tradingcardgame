...
type FiltersProps = {
    groupName: string
    /** `bar`: horizontal row (team builder). `sidebar`: compact grid for catalog left rail. */
    variant?: 'bar' | 'sidebar'
}

const Filters = ({ groupName, variant = 'bar' }: FiltersProps) => {
    const params = useSearchParams()
    const pathname = usePathname()
    const router = useRouter()
    const rawTypeFilter = params.get('type_filter')
    const selectedFilter = rawTypeFilter ? normalizeTypeFilterParam(rawTypeFilter) : ''
    ...
    const handleFilterChange = (filter: string) => {
        ...
        if (filter === '') {
            ...
            currentParams.set('type_filter', normalizeTypeFilterParam(filter))
    ...
    const btn = variant === 'sidebar' ? 'btn btn-md' : 'btn'
    const clearBtn =
        variant === 'sidebar'
            ? 'btn btn-error btn-xs normal-case col-span-2 w-full justify-center text-white'
            : 'btn btn-error btn-sm normal-case w-full basis-full justify-center text-white'
    const shell =
        variant === 'sidebar'
            ? 'filter grid grid-cols-2 gap-4'
            : 'filter mb-2 flex flex-row flex-wrap justify-start md:mb-0'
